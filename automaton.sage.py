...
# This file was *autogenerated* from the file automaton.sage
...
for j in range(number_of_marked_points):
    marpolygon = input("Starting from left to right, consider the %dth marked point, which has the singularity type %d. In counterclockwise order, enter the vertices that enclose this marked point as a list: " %(j+_sage_const_1 ,singularity_type["marked"][j]))
    ...
    while i < len(marpolygon):
        ...
        infpoly["marked"][j][_sage_const_1 ].append(vertex)
        side_swapper = tuple(sorted([int(side_swapper[_sage_const_1 ]), int(side_swapper[_sage_const_3 ])]))
        side_swapping_edges.append(side_swapper)
        i += _sage_const_2 


number_of_unmarked_points = int(input("Enter the number of unmarked singularities: "))
if number_of_unmarked_points != _sage_const_0 :
    ...
    for j in range(number_of_unmarked_points):
        unmarpolygon = input("Consider the %dth unmarked singularity, which has the singularity type %d. In counterclockwise order, enter the vertices that enclose this unmarked singularity as a list: " %(j+_sage_const_1 ,singularity_type["unmarked"][j]))
        i = _sage_const_1 
        while i < len(unmarpolygon):
            vertexi = int(unmarpolygon[i])
            infpoly["unmarked"][j] = infpoly["unmarked"][j] + tuple([[]])
            infpoly["unmarked"][j][_sage_const_1 ].append(vertexi)
            i += _sage_const_2 
else:
    unmarked_singularity_type = []
...
for i in range(num_cusps):
    ...
    left_edge = tuple(sorted([int(left[_sage_const_1 ]),int(left[_sage_const_3 ])]))
    right_edge = tuple(sorted([int(right[_sage_const_1 ]),int(right[_sage_const_3 ])]))
...
AutomatonGraph = DiGraph({_sage_const_0 :[]}) #The automaton as a graph
...
while we_added_new_traintracks == True:
    ...
    for j in list_of_tracks[number_of_traintrack_counter:]:
        ...
        for i in cusp_dictionary[list_of_tracks.index(j)]:
            ...
            if is_traintrack_in_list(T_ROL, list_of_tracks)[_sage_const_0 ] == False: #if folded T_ROL is NOT in list
            ...
            if is_traintrack_in_list(T_LOR, list_of_tracks)[_sage_const_0 ] == False:# if folded T_LOR is NOT in list
